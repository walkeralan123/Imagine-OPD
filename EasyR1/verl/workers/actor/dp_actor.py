...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        ...
        # Swap to teacher (ref) inputs for context distillation if present
        if "ref_input_ids" in micro_batch:
            ...
            if "ref_multi_modal_inputs" in micro_batch:
                multi_modal_inputs = batch_collate(micro_batch["ref_multi_modal_inputs"])
                multi_modal_inputs = {key: torch.cat(value, dim=0) for key, value in multi_modal_inputs.items()}
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        # Include ref keys for context distillation
        if "ref_input_ids" in data.batch:
            select_keys.extend(["ref_input_ids", "ref_attention_mask", "ref_position_ids"])
        if "ref_multi_modal_inputs" in data.non_tensor_batch:
            non_tensor_select_keys.append("ref_multi_modal_inputs")
