...
class FSDPWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._has_ref:
            if self.config.ref.sync_with_actor:
                if not self._has_actor:
                    raise ValueError("ref.sync_with_actor=True requires an actor module in the same worker.")
                self.ref_fsdp_module = self.fsdp_module
                self.print_rank0("Reference/teacher policy shares actor weights (sync_with_actor=True).")
            elif self._is_lora:
                self.ref_fsdp_module = self.fsdp_module
            else:
                self._build_model_optimizer(
    ...
    def compute_ref_log_probs(self, data: DataProto):
        ...
        # The ref module shares the actor module for LoRA and optional synced-teacher mode.
        shared_ref_module = self._is_lora or self.config.ref.sync_with_actor
        should_load_ref_params = self._use_param_offload if shared_ref_module else self._use_ref_param_offload
        if should_load_ref_params:
