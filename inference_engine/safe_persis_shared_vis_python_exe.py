...
# Import the sandbox execution backend
from inference_engine.sandbox import execute_code_in_sandbox, TEMP_PROCESSED_IMAGES_DIR
...
# Helper functions for message image handling
def _collect_image_urls_from_messages(messages: list) -> List[str]:
    """Helper to collect image URLs from message history."""
    image_urls: List[str] = []
    if not messages:
        return image_urls
...
def _inject_image_clues_into_context(
    execution_context: Dict[str, Any],
    messages: list,
    start_idx: int,
) -> int:
    """Injects image variables (image_clue_i) into the execution context locals."""
    if execution_context is None:
        return start_idx
    execution_context.setdefault('globals', {})
    execution_context.setdefault('locals', {})
    locals_dict = execution_context['locals']
    image_urls = _collect_image_urls_from_messages(messages)
    # Only process new images to maintain order
    for i in range(start_idx, len(image_urls)):
        var_name = f"image_clue_{i}"
        if var_name in locals_dict:
            continue
        img = base64_to_image(image_urls[i])
        if img is not None:
            locals_dict[var_name] = img
...
class PythonExecutor:
    def __init__(
        ...
        # Context management for sandbox
        self.execution_context: Dict[str, Any] = {'globals': {}, 'locals': {}}
        self._num_images_injected: int = 0
        self.persistent_worker = None # Placeholder for compatibility if accessed directly
        ...
        if self.use_process_isolation:
            try:
                if not isinstance(self.execution_context, dict):
                    self.execution_context = {'globals': {}, 'locals': {}}
                
                # 1. Inject images from messages into context
                self._num_images_injected = _inject_image_clues_into_context(
                    self.execution_context,
                    messages,
                    start_idx=self._num_images_injected,
                )
                locals_dict = self.execution_context.setdefault('locals', {})
                locals_dict.setdefault('_captured_figures', [])
                pre_figures_count = len(locals_dict['_captured_figures'])
                # 2. Prepare code (handle plt.show)
                code_str = "\n".join(code)
                show_replacement_body = (
                    "buf = io.BytesIO()\n"
                    "plt.savefig(buf, format='png')\n"
                    "buf.seek(0)\n"
                ...
                # Replace plt.show() with capture logic
                code_str = regex.sub(
                    r"(^[ \t]*)plt\.show\(\)[ \t]*;?.*$",
                    lambda m: "\n".join([m.group(1) + line for line in show_replacement_body.splitlines()]),
                    code_str,
                    flags=regex.MULTILINE,
                )
                # Add necessary imports for the replacement code
                header_lines = [
                    "import matplotlib",
                    "matplotlib.use('Agg')",
                    "import matplotlib.pyplot as plt",
                    "import io, base64",
                    "if '_captured_figures' not in locals(): _captured_figures = []",
                ]
                
                # Inject ImageRuntime HEADERS if available
                if self.runtime_class and hasattr(self.runtime_class, 'HEADERS'):
                     header_lines.extend(self.runtime_class.HEADERS)

                header = "\n".join(header_lines)
                final_code = header + "\n" + code_str
                # 3. Execute in Sandbox
                processed_paths_list, print_output, error_msg, new_ctx = execute_code_in_sandbox(
                    final_code,
                    input_image_path=input_image_path,
                    item_id="PythonExecutor",
                    temp_output_dir=TEMP_PROCESSED_IMAGES_DIR,
                    previous_execution_context=self.execution_context,
                )
                # 4. Update Context
                if isinstance(new_ctx, dict):
                    self.execution_context = new_ctx
                
                if error_msg:
                    return {'error': str(error_msg), 'traceback': ''}, f"Error: {str(error_msg)}"
                # 5. Collect Results
                locals_dict = self.execution_context.get('locals', {})
                all_figures = locals_dict.get('_captured_figures', [])
                new_figures = all_figures[pre_figures_count:] if isinstance(all_figures, list) else []
                # Add images saved to disk (returned by sandbox)
                if processed_paths_list:
                    for p in processed_paths_list:
                        if isinstance(p, str) and p.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')) and os.path.exists(p):
                            try:
                                new_figures.append(encode_image(p))
                            except Exception:
                                pass
                result_obj = {}
                if print_output:
                    result_obj['text'] = print_output
                if new_figures:
                    result_obj['images'] = new_figures
                return result_obj, 'Done'
            except Exception as e:
                return {'error': str(e), 'traceback': traceback.format_exc()}, f"Error: {str(e)}"
        
        else:
            # Legacy in-process execution (Original Logic)
            runtime = runtime_class(messages) if runtime_class else self.runtime_class(messages)
            try:
                if get_answer_from_stdout:
                    program_io = io.StringIO()
                ...
                captured_figures = runtime.captured_figures
                result_obj = {}
                if result: result_obj['text'] = result
                if captured_figures: result_obj['images'] = captured_figures
                return result_obj, "Done"
            except Exception as e:
                return {'error': str(e), 'traceback': traceback.format_exc()}, f"Error: {str(e)}"
    ...
    def batch_apply(self, batch_code, messages, input_image_path=None):
